...
protected:
    ...
    // Linear interpolation of two vec3s at given value u
    vec3 lp(const vec3& a, const vec3& b, double u);
    vec3 reduceAngle(vec3 key);
    vec3 convertToShortest(const vec3& key0, vec3 key1);
