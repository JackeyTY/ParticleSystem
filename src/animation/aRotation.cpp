...
void quat::FromRotation(const mat3& rot)
{
    ...
    double q0_abs = sqrt((1 + rot[0][0] + rot[1][1] + rot[2][2]) / 4);
    double q1_abs = sqrt((1 + rot[0][0] - rot[1][1] - rot[2][2]) / 4);
    double q2_abs = sqrt((1 - rot[0][0] + rot[1][1] - rot[2][2]) / 4);
    double q3_abs = sqrt((1 - rot[0][0] - rot[1][1] + rot[2][2]) / 4);
    double largestQ = fmax(fmax(q0_abs, q1_abs), fmax(q2_abs, q3_abs));
    if (fabs(q0_abs - largestQ) < DBL_EPSILON) {
        mQ[VW] = q0_abs;
        mQ[VX] = (rot[2][1] - rot[1][2]) / (4 * q0_abs);
        mQ[VY] = (rot[0][2] - rot[2][0]) / (4 * q0_abs);
        mQ[VZ] = (rot[1][0] - rot[0][1]) / (4 * q0_abs);
    }
    else if (fabs(q1_abs - largestQ) < DBL_EPSILON) {
        mQ[VW] = (rot[2][1] - rot[1][2]) / (4 * q1_abs);
        mQ[VX] = q1_abs;
        mQ[VY] = (rot[0][1] + rot[1][0]) / (4 * q1_abs);
        mQ[VZ] = (rot[0][2] + rot[2][0]) / (4 * q1_abs);
    }
    else if (fabs(q2_abs - largestQ) < DBL_EPSILON) {
        mQ[VW] = (rot[0][2] - rot[2][0]) / (4 * q2_abs);
        mQ[VX] = (rot[0][1] + rot[1][0]) / (4 * q2_abs);
        mQ[VY] = q2_abs;
        mQ[VZ] = (rot[1][2] + rot[2][1]) / (4 * q2_abs);
    }
    else {
        mQ[VW] = (rot[1][0] - rot[0][1]) / (4 * q3_abs);
        mQ[VX] = (rot[0][2] + rot[2][0]) / (4 * q3_abs);
        mQ[VY] = (rot[1][2] + rot[2][1]) / (4 * q3_abs);
        mQ[VZ] = q3_abs;
    }
	Normalize();
}
...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
    ...
    double omega = acos(quat::Dot(q0, q));
    q = (sin((1 - u) * omega) / sin(omega)) * q0 + (sin(omega * u) / sin(omega)) * q;
	return q.Normalize();
}
...
void quat::ToAxisAngle (vec3& axis, double& angleRad) const
{
    ...
    if (fabs(mQ[VW] - 1.0f) >= DBL_EPSILON) {
        angleRad = 2 * acos(mQ[VW]);
        vec3 v = vec3(mQ[VX], mQ[VY], mQ[VZ]);
        axis = v / v.Length();
    }
}
...
void quat::FromAxisAngle (const vec3& axis, double angleRad)
{
	...
	mQ[VW] = 1.0; mQ[VX] = 1.0; mQ[VY] = 0.0;  mQ[VZ] = 0.0;
...
mat3 quat::ToRotation () const
{
    ...
    double w = mQ[VW], x = mQ[VX], y = mQ[VY], z = mQ[VZ];
