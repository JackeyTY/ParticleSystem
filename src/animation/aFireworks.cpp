...
void AFireworks::fireRocket(vec3 pos, vec3 vel, vec3 color)
{
	...
	stateVals[4] = vel[1];
	stateVals[5] = vel[2];
	stateVals[7] = -GRAVITY * m_rocketMass;
...
void AFireworks::explode(vec3 pos, vec3 vel, vec3 color)
{
	...
	sparkNumber = (int)(rand() / (RAND_MAX / 50.f)) + 10;
	velocity = (((float)rand()) / (RAND_MAX / 20.f)) + 20.f;
	stateVals[0] = pos[0];
	stateVals[1] = pos[1];
	stateVals[2] = pos[2];
	stateVals[7] = -GRAVITY * m_sparkMass;
	stateVals[9] = m_sparkMass;
	stateVals[10] = SPARK_LIFESPAN;
	float theta = 2.f * PI / (sparkNumber * 1.f);
	vec3 localY = vel.Normalize();
	vec3 localZ = vec3(0.f, 0.f, 1.f);
	vec3 localX = localY.Cross(localZ).Normalize();
	for (int i = 0; i < sparkNumber; i++)
	{
		ASpark* newSpark = new ASpark(color);
		// TODO: Add your code here
		float curTheta = i * theta;
		vec3 curX = velocity * cos(curTheta) * localX;
		vec3 curY = velocity * sin(curTheta) * localY;
		stateVals[3] = (curX[0] + curY[0]);
		stateVals[4] = (curX[1] + curY[1]);
		stateVals[5] = (curX[2] + curY[2]);
