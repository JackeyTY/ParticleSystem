...
void AParticle::updateState(float deltaT, int integratorType)
{
	...
	switch (integratorType)
	{
		case EULER:
			...
			m_state[4] += m_stateDot[4] * deltaT;
			m_state[5] += m_stateDot[5] * deltaT;
			break;

		case RK2:
		{
			// Add your code here
			float vxp = m_state[3] + m_stateDot[3] * deltaT;
			float vyp = m_state[4] + m_stateDot[4] * deltaT;
			float vzp = m_state[5] + m_stateDot[5] * deltaT;
			m_state[0] += (m_stateDot[0] + vxp) * deltaT / 2.f;
			m_state[1] += (m_stateDot[1] + vyp) * deltaT / 2.f;
			m_state[2] += (m_stateDot[2] + vzp) * deltaT / 2.f;
			m_state[3] += m_stateDot[3] * deltaT;
			m_state[4] += m_stateDot[4] * deltaT;
			m_state[5] += m_stateDot[5] * deltaT;
			break;
